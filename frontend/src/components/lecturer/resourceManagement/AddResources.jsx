...
const AddResources = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append each file
      files.forEach((file) => {
        formData.append("files", file); // 'files' should match backend field name
      });
        ...
        <div className="fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-black/50">
          <div className="bg-white p-4 mx-4 sm:mx-0 sm:p-8 rounded-2xl w-full sm:w-[450px]">
            <div className="mb-6 flex items-center justify-between">
              <h1 className="text-xl font-medium ">Upload Resources</h1>
              <p
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
              <div className="flex flex-col gap-2">
                <label className="font-medium">Resource Title</label>
                <input
                  type="text"
                  name="title"
                  value={data.title}
              ...
              <button type="submit">
                <div className="mt-2 cursor-pointer bg-primaryColor text-white p-2 rounded-lg text-center hover:bg-primaryColor/80 duration-300 transition-all ease-linear">
                  <p className="font-medium">Upload now</p>
                </div>
              </button>
            </form>
          </div>
